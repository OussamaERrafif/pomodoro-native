...
export function getTokens(paletteId, darkMode) {
  const p = PALETTES[paletteId] || PALETTES.sunset;
  return darkMode ? p.dark : p.light;
}

export const FONT_DISPLAY = Platform.OS === 'ios' ? 'Georgia' : 'serif';
export const FONT_UI = undefined;
export const FONT_MONO = Platform.OS === 'ios' ? 'Menlo' : 'monospace';

export const PALETTE_IDS = ['sunset', 'clay', 'bloom', 'forest'];

export const AMBIENT_SOUNDS = [
  { id: 'rain', label: 'Rain', sub: 'Soft afternoon shower' },
  { id: 'cafe', label: 'Café', sub: 'Distant chatter, espresso machine' },
  { id: 'white', label: 'White noise', sub: 'Steady, neutral hum' },
  { id: 'forest', label: 'Forest', sub: 'Birds, leaves, faraway brook' },
  { id: 'fire', label: 'Fireplace', sub: 'Crackling embers' },
